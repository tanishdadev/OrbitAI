  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-purple-50">
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-4">
          ...
          <div className="h-96 overflow-y-auto p-4 bg-gray-50 rounded-lg">
            {responses.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-gray-500">
                <p className="text-lg font-medium">Your AI responses will appear here</p>
                <p className="text-sm mt-2">Try commands like:</p>
                <ul className="text-sm mt-2 space-y-1">
                  <li>• "Schedule a meeting tomorrow at 2pm"</li>
                  <li>• "Draft an email to john@example.com about project update"</li>
                ...
                {responses.map((response, i) => (
                  ...
                  >
                    ...
                    <div className={`whitespace-pre-wrap ${
                      response.type === 'command' ? 'text-blue-900' :
                      response.type === 'error' ? 'text-red-900' :
                      'text-gray-800'
                    }`}>
                      {response.type === 'command' ? `> ${response.text}` : response.text}
                    </div>
                  </div>
                ))}
              </div>
        ...
        {/* Command Prompt */}
        <CommandPrompt onSubmit={handleCommand} isProcessing={isProcessing} />
      </main>
    </div>
  )
}